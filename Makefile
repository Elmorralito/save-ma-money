...
VERSION ?= prerelease
MOD ?= ALL
PYTHON_CMD := $(shell if [ "$(POETRY_ACTIVE)" = "1" ] || [ -n "$(VIRTUAL_ENV)" ]; then echo "poetry"; else echo "python -m poetry"; fi)
...
activate: prep
	echo "eval \"$$($(POETRY_CMD) env activate)\"" | exec $(PBCOPY_COMMAND)
