WITH owner_monthly_net AS (
    SELECT
        t.owner_id,
        t.currency,
        EXTRACT(YEAR FROM t.transaction_ts)::INTEGER balance_year,
        EXTRACT(MONTH FROM t.transaction_ts)::INTEGER balance_month,
        SUM(
            CASE
                WHEN
    ...
    GROUP BY t.owner_id, balance_year, balance_month, t.currency
)
