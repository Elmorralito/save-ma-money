...
| Auth direction  | **Supabase Auth** via JWKS (`AUTH_PROVIDER=supabase`, PPT-039 / [#49](https://github.com/Elmorralito/save-ma-money/issues/49)); local HS256 remains for tests (`AUTH_PROVIDER=local`) — see `docs/issues/PPT-039-supabase-auth-reissue.md` |
...
**Auth:** `AUTH_PROVIDER=local` (HS256, tests/B0) or `supabase` (JWKS verify + provision-on-first-seen). See `core/security.py` / `dependencies/auth.get_current_owner` and [`ARCHITECTURE.md#part-vi--auth-contract-ppt-031-track-e`](../docs/design/ARCHITECTURE.md#part-vi--auth-contract-ppt-031-track-e).
