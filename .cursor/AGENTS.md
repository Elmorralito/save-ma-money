...
**Web setup (pnpm ≠ Poetry):** Node **22** + pnpm **9** via root `pnpm-workspace.yaml` / `packageManager`. Install with `pnpm install` (not `poetry install`). Day-to-day: `make web-dev` / `web-lint` / `web-test` / `web-build`. Packaging smoke: `make web-up` (nginx Compose). Do **not** port `papita_txnsmodel` rules into TypeScript — UI + TanStack Query + BFF session only. **Non-goal (PPT-069 / [#140](https://github.com/Elmorralito/save-ma-money/issues/140)):** never reimplement `UsersService` / model service business methods in JS; feature CRUD stays on existing API screens. SSOT: [`modules/web/README.md`](../modules/web/README.md) § Domain boundary · epic [#112](https://github.com/Elmorralito/save-ma-money/issues/112). Field RUM / Sentry deferred post-MVP; lab Lighthouse/CWV only (web README § Quality).
...
| `/transaction-templates` | `routers/v1/transaction_templates.py` | `TransactionTemplatesService` | CRUD + upcoming-dues / mark-paid / clear-paid ([#166](https://github.com/Elmorralito/save-ma-money/issues/166) PPT-073)                                                                   |
...
**Schemas:** `schemas/accounts.py`, `schemas/categories.py`, `schemas/transactions.py`, `schemas/transaction_templates.py`, `schemas/movements.py`, `schemas/reports.py`, `schemas/query_params.py`; enum slugs via `schemas/converters.py`.
