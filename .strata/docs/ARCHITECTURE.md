...
- **`modules/web`** — `@papita/web`: Vite + React 19 + TypeScript SPA (epic PPT-046 / [#112](https://github.com/Elmorralito/save-ma-money/issues/112)). Scaffold PPT-047 / [#113](https://github.com/Elmorralito/save-ma-money/issues/113); OpenAPI client PPT-048; BFF cookies PPT-049; design system + `AppLayout` PPT-051 / [#116](https://github.com/Elmorralito/save-ma-money/issues/116); accounts/categories UI PPT-052 / [#117](https://github.com/Elmorralito/save-ma-money/issues/117); transactions/movements UI PPT-053 / [#118](https://github.com/Elmorralito/save-ma-money/issues/118); reports spending PPT-054 / [#119](https://github.com/Elmorralito/save-ma-money/issues/119). pnpm workspace; presentation only — no domain logic. Dev proxy `/api` → `:8000`; nginx packaging PPT-057 / [#122](https://github.com/Elmorralito/save-ma-money/issues/122) (`docker/web/`, `make web-up`); CSP PPT-063 / #128. OpenAPI typegen **strategy B** (PPT-065 / [#130](https://github.com/Elmorralito/save-ma-money/issues/130)).
- **`bin/`** — shared shell utilities, `alembic.sh`, `test.sh`, `transaction_partitions.sh` (monthly partition ensure + retention archive).
- **`docker/`** — local PostgreSQL 15 + Redis 7 + API + nginx SPA via Compose (`docker/docker-compose.yml`, `docker/database/`, `docker/web/`).
...
| `transactions`                                     | Posted ledger (`transaction_kind`, `from_account_id`, `to_account_id`, `category_id`); **monthly RANGE(`transaction_ts`)** partitions (10-year retention; `config/transaction_partitions.py`, `bin/bash/transaction_partitions.sh`) |
