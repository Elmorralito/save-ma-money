...
- **`modules/web`** — `@papita/web`: Vite + React 19 + TypeScript SPA scaffold (PPT-047 / [#113](https://github.com/Elmorralito/save-ma-money/issues/113), epic PPT-046 / [#112](https://github.com/Elmorralito/save-ma-money/issues/112)). pnpm workspace at repo root; presentation only — no domain logic. Dev proxy `/api` → `:8000`; quality via `web-ci.yml`. Feature screens / BFF / nginx are later epic children.
