...
- **`modules/web`** — `@papita/web`: Vite + React 19 + TypeScript SPA (epic PPT-046 / [#112](https://github.com/Elmorralito/save-ma-money/issues/112)). Scaffold PPT-047; OpenAPI client PPT-048; BFF cookies PPT-049; design system + `AppLayout` PPT-051; accounts/categories UI PPT-052; **transactions/movements UI PPT-053 / [#118](https://github.com/Elmorralito/save-ma-money/issues/118)** (`api/transactions.ts`, `api/movements.ts`, Idempotency-Key, bulk cap, pending execute/cancel). pnpm workspace; presentation only — no domain logic. Dev proxy `/api` → `:8000`; quality via `web-ci.yml`. OpenAPI typegen **strategy B** (PPT-065 / [#130](https://github.com/Elmorralito/save-ma-money/issues/130)). Dashboard/reports, forms kit, nginx are later epic children.
