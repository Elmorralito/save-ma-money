...
| Supabase B1 Smoke    | [`workflows/supabase-b1-smoke.yml`](./workflows/supabase-b1-smoke.yml)   | Push to `main`; `workflow_dispatch`                        | Optional pooler smoke when `SUPABASE_B1_DATABASE_URL` set |
...
4. **Pytest + coverage** via [`deploy/test.sh`](../deploy/test.sh)
   ...
   - Env: `AUTH_PROVIDER=local` (Supabase Auth smoke is manual: `make auth-smoke`)
...
### Supabase B1 Smoke (optional, PPT-040)

| Field        | Detail                                                                                                                                  |
| :----------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**  | Push to `main`; `workflow_dispatch`                                                                                                     |
| **Secret**   | `SUPABASE_B1_DATABASE_URL` — Supabase transaction pooler (`:6543` / `*.pooler.supabase.com`)                                            |
| **Behavior** | If secret unset → job succeeds with skip message. If set → `AUTH_PROVIDER=local` + `pytest modules/api/tests/test_supabase_b1_smoke.py` |
| **Local**    | `PAPITA_ENV=staging make b1-smoke` (or `./deploy/b1_smoke.sh`) — fails loudly when the pooler gate is not met                           |
