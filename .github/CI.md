...
| Publish model (PyPI) | [`workflows/publish-model.yml`](./workflows/publish-model.yml)           | Push `main`→TestPyPI / `release`→PyPI (model paths); dispatch | Build + OIDC publish `papita-transactions-model` (PPT-024) |
...
|             |                                                                                     |
| :---------- | :---------------------------------------------------------------------------------- |
| **Trigger** | Push **`main`** → TestPyPI; push **`release`** → PyPI; optional `workflow_dispatch` |
| **Paths**   | `modules/model/**`, `bin/package.sh`, `.github/workflows/publish-model.yml`         |
...
| **Gates**   | Clean-venv wheel import smoke; `skip-existing` if version already on the index      |

**Release flow:** bump `modules/model/pyproject.toml` → merge packaging changes to `main` (TestPyPI) → promote to `release` (PyPI).

**Operator setup (once):** on [TestPyPI](https://test.pypi.org/) / [PyPI](https://pypi.org/), add a Trusted Publisher for this repository, workflow `publish-model.yml`, and the matching environment. Restrict to branches `main` / `release` as appropriate. Prefer environment protection on `pypi`. Do not store long-lived tokens in git.
