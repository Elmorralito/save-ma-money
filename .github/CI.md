...
| Ingestor CI           | [`workflows/ingestor-ci.yml`](./workflows/ingestor-ci.yml)                     | PR + push to `main` (`modules/ingestor*/**`, `bin/make/ingestor.mk`, `docker/ingestor/**`, `pyproject.toml`)                    | Poetry install (+ `ingestor-prefect`) + lint/test + `docker build` smoke for `docker/ingestor` (PPT-082 / #176); cov ≥80% → PPT-084               |
...
|                 |                                                                                                                                                                                                   |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**     | Path-relevant push to **`main`** → stable GHCR; **PR** → dev GHCR only if web runtime paths changed (not e2e/docs/workflow-only; unless `skip-web-image-dev`); dispatch `smoke-only` \| `publish` |
| **Jobs**        | `publish` (Environment **`ghcr`**) · `publish-dev` (Environment **`ghcr-dev`**, gated by `changes.web`) · `changes` · `build-smoke` (dispatch only)                                               |
...
|                 |                                                                                                                                                                                                |
| :-------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**     | Path-relevant push to **`main`** → stable GHCR; **PR** → dev GHCR only if API/model runtime paths changed (not workflow-only; unless `skip-api-image-dev`); dispatch `smoke-only` \| `publish` |
| **Jobs**        | `publish` (Environment **`ghcr`**) · `publish-dev` (Environment **`ghcr-dev`**, gated by `changes.api`) · `changes` · `build-smoke` (dispatch only)                                            |
